...
// doomgeneric's doomtype.h has PACKEDATTR but dropped PACKED_STRUCT.
...
static void I_OPL_SetMusicVolume(int volume)
{
    ...
    // Hold the callback lock so the higher-priority PS2 mixer thread can't fire
    // MIDI events (which reorder the voice lists) while we walk them.
    OPL_Lock();
...
static void I_OPL_PlaySong(void *handle, boolean looping)
{
    ...
    // Serialize against the mixer thread firing MIDI callbacks: without this,
    // the higher-priority mixer can fire a scheduled event (keying voices,
    // reading a track iterator) before we've finished setting tracks/channels
    // up, corrupting playback so the song silently stops.
    OPL_Lock();
...
{
    // PS2: no writable filesystem, so we load entirely from memory (the stock
    // path writes a temp .mid and re-reads it). MUS is converted to MIDI in a
    // memio buffer, then loaded via fmemopen + MIDI_LoadStream.
    ...
    {
        // Already a MIDI: load straight from the supplied buffer.
        f = fmemopen(data, len, "rb");
        if (f != NULL)
        {
            result = MIDI_LoadStream(f);
            fclose(f);
        }
    }
    else
    {
        // Assume a MUS lump: convert to MIDI in memory, then load.
