...
// PS2 is little-endian, so big-endian swap == byte swap. Avoid the SDL dep.
#ifndef SDL_SwapBE16
...
// chocolate helpers doomgeneric doesn't provide.
#ifndef M_fopen
#define M_fopen(path, mode) fopen((path), (mode))
#endif
#ifndef I_Realloc
#define I_Realloc(ptr, size) realloc((ptr), (size))
#endif
...
// Load a MIDI from an already-open stream (does NOT close it). Lets the PS2
// port load from memory via fmemopen without a temp file.
midi_file_t *MIDI_LoadStream(FILE *stream)
{
    midi_file_t *file;
